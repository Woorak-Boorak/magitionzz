...
#define FPGA_ONCHIP_BASE_0      0x09000000
#define FPGA_ONCHIP_BASE_1      0x0a000000
#define FPGA_ONCHIP_SPAN      0x00000FFF
#define HW_REGS_SPAN          0x00005000
#define HW_REGS_BASE          0xff200000

#define FPGA_ONCHIP_END       0x08000FFF
#define FPGA_ONCHIP_END_0       0x09000FFF
#define FPGA_ONCHIP_END_1       0x0a000FFF
...
int read_csv(const char *filename, float * weights, int nrow, int ncol) {
      ...
      printf("Attempting to load CSV file: %s\n", filename);
      fp = fopen(filename, "r");
      if (fp == NULL) {
...
void loadBMP(const char *filename, float *image_converted, int nrow, int ncol) {
    ...
    printf("BMP file opened successfully: %s\n", filename);
...
int main(){
    ...
    printf("Mapping HW_REGS_BASE at 0x%x with span 0x%x\n", HW_REGS_BASE, HW_REGS_SPAN);
    ...
    printf("HW_REGS_BASE mapped successfully.\n");
   ...
   //이미 가중치들이 고정소수점 방식임
	...
	while(exit != 2535){
        if (count == 0){
            printf("Starting to write input image data to FPGA SRAM_0...\n");
            for(yjc = 0;yjc<28*28;yjc++){
                //*(sram_ptr + i) = rand();
                *(sram_ptr_0 + yjc) = input_image_fpga[yjc];
                    if (yjc % 100 == 0) {  // 중간 진행 상태 출력
                        printf("Writing to SRAM_0: index %d, value %d\n", yjc, input_image_fpga[yjc]);
                    }
            }
            ...
            printf("Convolution weights written to SRAM_1 successfully.\n");
            count = 1;
            printf("conv_done");
        }
        printf("Waiting for load_sign signal from FPGA...\n");
        if (*load_sign == 1){
            printf("load_sign received. Starting to send FC weights to FPGA.\n");
			for(yjc = 0;yjc<2535;yjc++){
				*(sram_ptr + yjc) = weights_fc_fpga[yjc+2535];
				*(sram_ptr_1 + yjc) = weights_fc_fpga[yjc];
                printf("FC weights written: index %d, value (sram_ptr) %d, value (sram_ptr_1) %d\n", yjc, weights_fc_fpga[yjc + 2535], weights_fc_fpga[yjc]);
				exit++;
                if(exit==2535){
                    printf("All FC weights sent. Setting fc_sign.\n");
                    *fc_sign = 1;
                }
			}
		}
